...
const AdminDashboard = () => {
  ...
  const [newMakanan, setNewMakanan] = useState({ name: '', description: '', link: '' });
  const [isEditing, setIsEditing] = useState(false);
  const [currentMakananId, setCurrentMakananId] = useState(null);
  ...
  const navigate = useNavigate();
    ...
    const handleAddMakanan = async (e) => {
      e.preventDefault();
      if (!newMakanan.link) {
        alert("Tunggu sebentar, gambar masih diunggah!");
        return;
      }
      const docRef = await addDoc(collection(db, 'makanan'), newMakanan);
      setMakanan([...makanan, { id: docRef.id, ...newMakanan }]);
      setNewMakanan({ name: '', description: '', link: '' });
    ...
    const startEditingMakanan = (item) => {
      setIsEditing(true);
      setNewMakanan({ name: item.name, description: item.description, link: item.link });
      setCurrentMakananId(item.id);
      setSelectedImage(item.link);
      setIsPopupOpen(true);
    };

    const handleUpdateMakanan = async (e) => {
      e.preventDefault();
      const makananRef = doc(db, 'makanan', currentMakananId);
      await updateDoc(makananRef, newMakanan);

      setMakanan(makanan.map((item) => (item.id === currentMakananId ? { id: currentMakananId, ...newMakanan } : item)));
      setIsEditing(false);
      setNewMakanan({ name: '', description: '', link: '' });
      setCurrentMakananId(null);
      setSelectedImage(null);
      setIsPopupOpen(false);
    };
      ...
      const handleImageUpload = async (e) => {
        ...
        try {
          ...
          if (data.secure_url) {
            ...
            setNewMakanan((prev) => ({ ...prev, link: data.secure_url }));
          } else {
        ...
        <div className="admin-dashboard bg-paper flex grid-cols-2">
          ...
          <div className='flex flex-col w-full mt-10 mr-5 ml-5'>
            <div className="content flex mb-5">
              <h2 className='font-jdl text-5xl text-bckg'>MAKANAN</h2>
            ...
            {isPopupOpen && (
              <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50">
                <div className="bg-white p-6 rounded-xl shadow-lg">
                  <h2 className="text-lg font-bold mb-2">{isEditing ? "Edit Makanan" : "Tambah Makanan"}</h2>
                  <form onSubmit={isEditing ? handleUpdateMakanan : handleAddMakanan} className='space-y-1 flex'>
                  <div className="relative w-full mr-5">
                    ...
                    <input type="text" placeholder='' value={newMakanan.name} onChange={(e) => setNewMakanan({ ...newMakanan, name: e.target.value })} className='w-full p-2 border rounded-xl bg-gray-200' required />
                    <div className="relative space-y-1">
                      <label className="text-txtnv pb-3 font-mnu text-md font-thin">Harga</label>
                      <input
                        type="text"
                        value={newMakanan.description ? `Rp ${newMakanan.description}` : ''}
                        onChange={(e) => {
                          const value = e.target.value.replace(/\D/g, ''); // Hanya angka
                          setNewMakanan({ ...newMakanan, description: value });
                        }}
                        placeholder="Rp 0"
                ...
                {makanan.map(item => (
                  <li key={item.id} className='flex items-center bg-white rounded-xl shadow-md overflow-hidden'>
                    ...
                    <div className="flex flex-col items-center pr-3 gap-2">
                      <button onClick={() => startEditingMakanan(item)} className='text-gray-500 hover:text-yellow-500'>
                        <FontAwesomeIcon icon={faPenToSquare} size="lg" />
                      </button>
                      <button onClick={() => handleDeleteMakanan(item.id)} className='text-red-500 hover:text-red-700'>
